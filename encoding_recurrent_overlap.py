...
class EncodingRecurrentOverlap(object):

	def __init__(self, n_vis, n_hid, layer_name, rng=None, encoding_length=10, overlap_length=3, param_init_range=0.02):

		self.n_vis = n_vis
		self.n_hid = n_hid
		self.layer_name = layer_name
		assert encoding_length > overlap_length, 'encoding_length must be greater than overlap_length'
	...
	def encode(self, state_below):
		...
		"""
		state_below = self.overlapping_window(state_below, self.encoding_length, self.overlap_length)
	...
	def decode(self, encodings):
		...
		:type rval: theano matrix (2d tensor)
		:param rval: the reshaped, reconstructed input
		"""
		Whhd = self.Whhd
		Wxh = self.Wxh
		Whx = self.Whx
		...
		# each row of encodings is an encoding for a given set of inputs
		# each encoding is responsible for reconstructing encoding_length inputs
		# reconstructed_input here is of shape (n_encodings, encoding_length, n_vis)
	...
	def overlapping_window(self, seq, window_length, overlap_length):
		"""
		:development: Length specifies the length of the dataset samples. This is a valid constraint b/c the length of the sequences must be constant as enforced by theano anyway. Even when using variable length seqeunces with mask values, this assump holds. 
		"""
		length = 31
		#return T.concatenate([seq[idx:idx + window_length] for idx in range(length) if T.mod(idx, window_length - overlap_length) == 0 and seq[idx: idx + window_length].shape[0] == window_length])
		# return T.concatenate([seq[idx:idx + window_length] for idx in range(length) if idx % window_length - overlap_length == 0])

		subseqs = []
		for idx in range(length - (length % window_length - overlap_length)):
			if idx % window_length - overlap_length == 0:
				subseqs.append(seq[idx: idx + window_length])
		return T.concatenate(subseqs)
	...
	def get_pretraining_cost_updates(self, input_sequence, learning_rate=0.01):
		"""
		...
		# compare to an input sequence that has been extended
		extended_input_sequence = self.overlapping_window(input_sequence, self.encoding_length, self.overlap_length)
		cost = T.sum(T.sqr(extended_input_sequence - reconstructed_input_sequence))
